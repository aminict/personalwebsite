           ...
           @if (Session ::has('success'))
            ...
            @if (Session ::has('danger'))
             ...
             @endif
					...
					<div class="row">
						<div class="col-lg-12">
              <input class="form-control" type="text" name="message" placeholder="Default input">
						</div>
					</div>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>{!! $row->message !!}</td>
                   ...
                   <td><img src="{{ URL::to('/') }}/sliderimage/{{ $row->image }}" class="img-thumbnail" width="75" /></td>

                  <td>{{$row->publish}}</td>
                  <td><a class="btn btn-primary" href="{{route('slider.edit',$row->id)}}" role="button">Edit</a></td>
                  <td>
                  	<form action="{{route('slider.destroy',$row->id)}}" method="POST" onsubmit="return confirm('Are you sure ?')">
                     @csrf
                     @method('delete')
